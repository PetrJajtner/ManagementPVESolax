 ...
 * @copyright  Ing. Petr Jajtner 2019
 */
...
export function versionSort(versionA: string, versionB: string): number {
  ...
  for (let i = 0, m = Math.max(a.length, b.length); i < m; i++) {
    ...
    if (0 !== (+(a[i] as string) - +(b[i] as string))) {
      return +(a[i] as string) - +(b[i] as string);
    }
  }
  return 0;
}
 ...
 */
export function sort<T extends object, K extends keyof T, V extends T[K]>(arr: T[], key: string, convert?: (v: unknown) => V): T[] {
  let descending = 1;
  ...
  return [...arr].sort((a: T, b: T) => {
    if (isObject(a) && key in a && isObject(b) && key in b) {
      const
        aValue = convert ? convert(a[key as K]) as V : a[key as K] as V,
        bValue = convert ? convert(b[key as K]) as V : b[key as K] as V
      ;
      if (aValue < bValue) {
        return -descending;
      }
...
export function UUID(): string {
  const hexDigits = '0123456789abcdef', result: string[] = new Array<string>(36);

  for (let i = 0; i < 36; ++i) {
    result[i] = hexDigits[Math.floor(Math.random() * 0x10)] as string;
  }

  result[19] = hexDigits[(Math.floor(Math.random() * 0x10) & 0x3) | 0x8] as string;
